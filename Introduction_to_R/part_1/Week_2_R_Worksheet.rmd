...
for(x in vector){
  if(x > 50) {
    print(paste0(x, "above 50"))
    above_50 <- c(above_50, x)
  } else {
    print(paste0(x, "below 50"))
    below_50 <- c(below_50, x)
  }
}
...
dob <- as.Date(readline(prompt = "Please enter your age \nUsefromat YYYY-MM-DD"))
...
age_in_days <- as.numeric(difftime(today, dob, units="days"))
...
i <- 0 
for (gene_exp in treatment){
  i <- i +1
  print(paste0("Gene_", i, " has an expression value of ", gene_exp ))
}
